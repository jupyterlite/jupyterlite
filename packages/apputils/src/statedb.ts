 ...
 */
export class IndexedDBDataConnector implements IDataConnector<string> {
  ...
  async fetch(id: string): Promise<string> {
    return (await (await this.storage).getItem(id)) as string;
  }
  ...
  async list(namespace = ''): Promise<{ ids: string[]; values: string[] }> {
    ...
    (await storage.keys()).forEach(async (key) => {
      // Casting to string here, we know for sure the item is there
      items[key] = (await storage.getItem(key)) as string;
    });
